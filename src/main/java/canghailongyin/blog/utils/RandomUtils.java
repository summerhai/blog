...
public class RandomUtils {
    ...
    public static String getRandomCerCode() {
        StringBuilder generater = new StringBuilder();
        while (generater.length() != 18) {
...
//							: "invalid"));
    ...
    public static String getRandomJiGuan() {
        int index = getNum(0, areaCode.size() - 1);
//        System.out.println(index);
        int ran = 0;
        for (Map.Entry<String, Integer> entry : areaCode.entrySet()) {
            if (ran == index) {
                int value = entry.getValue();
                String newValue = String.valueOf(value).substring(0, 2) + "0000";
                String province = getKeyFromMap(newValue);
                String city = getCityFromMap(newValue);
    ...
    private static String getCityFromMap(String newValue) {
        String prefix = newValue.substring(0, 2);
        List<String> citys = new ArrayList<String>();
        for (Map.Entry<String, Integer> entry : areaCode.entrySet()) {
            int value = entry.getValue();
            if (String.valueOf(value).startsWith(prefix) && !String.valueOf(value).endsWith("0000")) {
                citys.add(entry.getKey());
            }
        }
        int randomIndex = getNum(0, citys.size() - 1);
        if (citys.size() == 0) {
            return null;
        }
        return citys.get(randomIndex);
    }
...
//        DatabaseConnection connection = new DatabaseConnection("ww");
//        try {
...
//            sql = "select distinct county as city from administrative_division where province='" + province + "'";
//        } else {
//            sql = "select distinct municipal as city from administrative_division where province='" + province + "'";
//        }
//        List<String> citys = new ArrayList<>();
//        try {
//            ResultSet set = connection.get(sql);
//            while (set.next()) {
...
//    }
    ...
    public static String getRandomCity() {
        ...
        for (Map.Entry<String, Integer> entry : areaCode.entrySet()) {
            if (ran == index) {
                int value = entry.getValue();
                if (!String.valueOf(value).endsWith("0000")) {
                    return getKeyFromMap(String.valueOf(value));
                } else {
                    return "海淀区";
                }
            }
    ...
    private static String getKeyFromMap(String newValue) {
    ...
    public static String getSuspectContent(String p2pName) {
        String curDate = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(new Date());
        String[] contentKeyWords = {"上访", "个访", "群访", "维权", "抗议", "集会", "围堵", "集体上访", "上访闹事",
                "越级上访", "赴京上访", "联名上访"};
        String[] what = {"什么时候去", "都买票了吗去", "都谁去", "大家做好准备", "集合地点在A，然后去"};
        String[] preP2p = {"关于", "请问", "求教", "都有谁", "投资了", "被坑了", "有一起的吗"};
        String[] when = {"9月25号上午九点", "9月25号上午九点三十", "9月26号中午12点半", "2017.9.23 12:30", "25号8:30", "26日下午一点整",
                "9月27日晚上六点", "2017年9月25号上午九点", "2017年9月25日上午九点三十", "2017年9月26号中午12点半", "9.23号12:30", "25日18:30",
                "9月27日晚上18:00"};
        String[] where = {"国家信访局附近", "信访局周围", "公安部附近", "公安部集访维权办旁边", "公安厅周围", "大会堂附近", "天安门附近", "北京", "十九大时候", "两会期间"};
        String[] action = {"集合", "准备", "碰头", "汇合", "见面", "约定"};

        int randomNum1 = getNum(0, what.length - 1);
        if (randomNum1 >= what.length)
            randomNum1 = 1;
        int randomNum2 = getNum(0, preP2p.length - 1);
        if (randomNum2 >= preP2p.length)
            randomNum2 = 1;
        int randomNum3 = getNum(0, contentKeyWords.length - 1);
        if (randomNum3 >= contentKeyWords.length)
            randomNum3 = 0;
        int randomNum4 = getNum(0, when.length - 1);
        if (randomNum4 >= when.length)
            randomNum4 = 0;
        int randomNum5 = getNum(0, where.length - 1);
        if (randomNum5 >= where.length)
            randomNum5 = 0;
        int randomNum6 = getNum(0, action.length - 1);
        if (randomNum6 >= action.length)
            randomNum6 = 0;
        int randomNum0 = getNum(1, 100);
        String result = "";
        if (randomNum0 % 2 == 0) {//聊天内容类似：被坑了E租宝,集合地点在A，然后去集体上访
            result = preP2p[randomNum2] + p2pName + "," + what[randomNum1] + contentKeyWords[randomNum3];
        } else {//聊天内容类似：26日下午一点整,在公安厅周围准备,准备群访
            result = when[randomNum4] + ",在" + where[randomNum5] + action[randomNum6] + ",准备" + contentKeyWords[randomNum3];
        }
        return result;
    }

    public static String getRandomTime(String visitTimeBase, int num) throws ParseException {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        long seconds = format.parse(visitTimeBase).getTime() / 1000;
        String time = format.format(new Date((seconds + num) * 1000));
        return time;
    }

    public static String getRandomTime(String startTime, String endTime) throws ParseException {

        long seconds1 = format.parse(startTime).getTime() / 1000;
        long seconds2 = format.parse(endTime).getTime() / 1000;
        long randomSeconds = getNum((int) seconds1, (int) seconds2);
        String time = format.format(new Date(randomSeconds * 1000));
        return time;
    }

    /**
     * 计算当前时间下，距离start开始了多久
     *
     * @param startDate
     * @return
     */
    public static long caculateTime(Date startDate) throws ParseException {
        long seconds1 = new Date().getTime();
        long seconds2 = startDate.getTime();
        return (seconds1 - seconds2) / 1000;
    }
}
